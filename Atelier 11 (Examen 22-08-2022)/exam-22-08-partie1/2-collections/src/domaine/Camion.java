...
public class Camion  {
    ...
    public boolean ajouterTrajet(Trajet trajet){
        ...
        for (LocalDate date : trajets.keySet()){
            Trajet trajIter = trajets.get(date);
            if (date.equals(trajet.getDate())) return false;
            if (date.plusDays(1).equals(trajet.getDate())
                    && !trajIter.getVilleArrivee().equals(trajet.getVilleDepart()))
                return false;
            if (date.minusDays(1).equals(trajet.getDate())
                    && !trajIter.getVilleDepart().equals(trajet.getVilleArrivee()))
                return false;
    ...
    public Trajet trouverTrajet(LocalDate dateTrajet){
       /* for (LocalDate date : trajets.keySet()) {
            if (trajets.get(date).getDate().equals(dateTrajet)) return trajets.get(date);
    ...
    private Trajet rechercherTrajet(Caisse caisse) {
        Trajet trajet = null;
        for (LocalDate date: trajets.keySet()){
            Trajet trajetPrevu = trajets.get(date);
