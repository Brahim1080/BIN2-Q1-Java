...
public class Camion  {
    ...
    public List<Trajet> trouverTrajetsAvecPlaceRestante(){
        return trajets.stream()
                .filter(caisses -> caisses.nbCaisses()<nbMaxCaisses).sorted(
                       (t1,t2)->{
                           if (t1.getVilleDepart().equals(t2.getVilleDepart()))
                               return t1.getVilleArrivee().compareTo(t2.getVilleArrivee());
                           return t1.getVilleDepart().compareTo(t2.getVilleDepart());
                       }
                )
    ...
    public LocalDate trouverDateTrajet(String villeDepart, String villeDArrivee){
        return trajets.stream()
                .filter(trajet->
                        trajet.getVilleDepart().equals(villeDepart) &&
                                trajet.getVilleArrivee().equals(villeDArrivee)&&
                                trajet.getDate().isAfter(LocalDate.now())
                ).map(Trajet::getDate).reduce((a,b)->{
                    if (a.isBefore(b))
                        return a;
                    return b;
                }).orElse(null);
